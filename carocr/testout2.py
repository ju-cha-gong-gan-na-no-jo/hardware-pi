...
if 'pi' in sys.argv[1]:
    ...
    data_carnum = {"car_num" : charss, "time" : current_times}
    res = requests.post('http://15.165.153.54:3000/out', data=json.dumps(data_carnum), headers=headers)
    # print(str(res.status_code) + " | " + res.text)
    # client.publish('common', json.dumps({"type": "2","car_num" : charss, "time" : current_times}), 1)

    # client.publish('common', json.dumps({"car_num" : charss, "time" : current_times}), 1)
else:
    img_name = charss + '_out.jpg'
    headers = {'Content-Type': 'application/json; charset=utf-8'}
    data_carnum = {"type":"2","car_num" : charss, "time" : current_times}
    res = requests.post('http://15.165.153.54:3000/out', data=json.dumps(data_carnum), headers=headers)
